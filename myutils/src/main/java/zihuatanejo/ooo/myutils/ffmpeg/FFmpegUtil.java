...
public class FFmpegUtil {
    ...
    public static  String[] cutAudio(String srcFile, long startTime, long duration, String targetFile){
     ...
     */
    @SuppressLint("DefaultLocale")
    public static  String[] mediaMux(String videoFile, String audioFile, long duration, String muxFile){
    ...
    public static  String[] extractAudio(String srcFile, String targetFile){
        //-vn:video not
        String mixAudioCmd = "-i %s -vn -y -acodec copy %s";//"ffmpeg -i %s -vn -y -acodec copy %s";
...
//        String transformVideoCmd = "ffmpeg -i %s -r 25 -b 200 -s 1080x720 %s";
    ...
    /**
     * 为视频添加关键帧(可以有效解决播放视频seekTo不准确的问题)
     * @param sourceFile
     * @param targetFile
     * @return
     */
    public static String[] addPrimaryKeyInt(String sourceFile, String targetFile){
        String toImage = "-i %s -c:v libx264 -preset superfast -x264opts keyint=24 -acodec copy -f mp4 %s";
        toImage = String.format(toImage, sourceFile, targetFile);
        Logger.wtf("为视频添加关键帧:%s",toImage);
     ...
     */
    @SuppressLint("DefaultLocale")
    public static  String[] pictureToVideo(String sourceFile, String targetFile){
        //-f image2：代表使用image2格式，需要放在输入文件前面
        String combineVideo = "ffmpeg -f image2 -r 20 -i %s -vcodec mpeg4 %s";//"ffmpeg -f image2 -r 1 -i %simg#d.jpg -vcodec mpeg4 %s";
        combineVideo = String.format(combineVideo, sourceFile, targetFile);
        combineVideo = combineVideo.replace("#", "%");
        Log.e("VideoHandleActivity", "combineVideo=" + combineVideo);
        return combineVideo.split(" ");//以空格分割为字符串数组
    }

    /**
     * 带音频合成视频命令
     * @param sourceFile 图片列表目录
     * @param audioFile 音频地址
     * @param targetFile 输出位置
     * @return
     */
    public static String[] pic2VideoWithAudio(String sourceFile, String audioFile, String targetFile){
        String cmd = "-threads 2 -y -r 20 -i %s -i %s -absf aac_adtstoasc %s";
        cmd = String.format(cmd,sourceFile,audioFile,targetFile);
        cmd = cmd.replace("#","%");
        Logger.wtf("VideoHandleActivity带音频的图片合成视频:%s",cmd);
        return cmd.split(" ");
    }
...
//        String reverseVideo = "ffmpeg -i %s -filter_complex [0:v]reverse[v];[0:a]areverse[a] -map [v] -map [a] %s";
     ...
     * @param channel 声道数 1单声道 2双声道
     * @param biteRate 音频比特率 16k
     * @param vol 音量 1-100
     * @param format 转换格式 wav/aac/mp3
     * @return
     */
    public static String[] audioFormat(String inputFile, String outputFile, int sampleRate, int channel, String biteRate, int vol, String format){
        String formatStr = "-i %s -ar %d -ac %d -ab %s -vol %d -f %s %s";
        formatStr = String.format(formatStr,inputFile,sampleRate,channel,biteRate,vol,format,outputFile);
